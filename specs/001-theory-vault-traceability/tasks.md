...
- [ ] T032 [US3] Implement `tools/vaultcheck/__main__.py` and `tools/vaultcheck/__init__.py` — argument parsing (`--root`, `--vault`, `--ignore-file`, `--quiet`, `--version`), orchestration of the rule modules, and the summary line; both `vaultcheck` and `python -m tools.vaultcheck` must work. Per ADR-0003, print a warning (not a failure) when run in a clone whose `core.hooksPath` is not `.githooks`, so an uninstalled local hook is visible

...
- [ ] T014 [US1] (Author trial done 2026-09-21, see docs/vault/logs/log-2026-09-21-reader-trial-adr-0001.md; stays open for a stranger.) Run the reader test from the Independent Test above and record the outcome, including anything the reader could not follow, in `docs/vault/logs/`
...
- [X] T038 [P] [US4] Write one literature note per candidate reference space in `docs/vault/literature/`: Universal Ventricular Coordinates, Universal Atrial Coordinates, the AHA 17-segment model, atrial segment models, fixed-atlas vertex indexing, and hybrid schemes, each stating coverage, licence, availability and consequences for pooling (FR-024)
- [X] T039 [P] [US4] (Done 2026-09-25. Cardiac Atlas Project surveyed 2026-09-24, licence unknown. Second pass 2026-09-25 found Attribution-licensed alternatives for both chambers: see rodero-four-chamber-meshes, nagel-biatrial-shape-model and other-open-cardiac-meshes.) Write one literature note per open cardiac atlas found, with licence and anatomical coverage (FR-025)
- [X] T040 [P] [US4] Write one literature note per commercial system: CARTO 3, Affera, EnSite X EP, Rhythmia HDx, KODEX-EPD, recording what each can export and under what licence terms that export may be read, from published documentation only (FR-026). For CARTO 3 and Affera also record display conventions (colour scales, standard views, orientation labels), since ADR-0006 gives those two a viewer mode. Check the OpenEP project first: it carries a CARTO importer under Apache-2.0
...
## Phase 8: Writing style (ADR-0005), added 2026-09-21

**Origin**: author reader trial of ADR-0001 (T014). Not in the original plan.

- [X] T049 Write `docs/vault/decisions/ADR-0005-writing-style.md` before any enforcement code
- [X] T050 Add `tools/vaultcheck/style.py` (STYLE-EM-DASH, STYLE-CURLY-QUOTE, STYLE-AI-ARTIFACT errors; STYLE-AI-VOCABULARY warning), count table rows as options, one fixture per new rule plus a table-options control
- [X] T051 Reformat vault notes into tables and flowcharts with decision content unchanged, and remove every em dash from specs, constitution (v1.2.2), README and code
